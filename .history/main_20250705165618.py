...
if __name__ == "__main__":
    ...
    if debris_events:
        ...
        # Affichage du point de croisement prédit
        cross_times = [t + tmin for t, tmin in zip(t_debris, tmin_debris)]
        cross_sat = [alt + v * tmin for alt, v, tmin in zip(alt_debris, estimated_velocities, tmin_debris)]
        cross_debris = [debris_states[idx]['pos'][0] + debris_states[idx]['vel'][0]*(t + tmin)
                        for idx, t, tmin in zip(idx_debris, t_debris, tmin_debris)]
        plt.scatter(cross_times, cross_sat, c='red', marker='*', s=120, label='Position satellite au croisement')
        plt.scatter(cross_times, cross_debris, c='purple', marker='P', s=100, label='Position débris au croisement')
