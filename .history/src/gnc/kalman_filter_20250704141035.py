...
class SatelliteTracker:
    ...
    Args:
        ...
        min_altitude (float): Minimum allowable altitude in km (default: 150.0, avoids non-physical values).
    """
    def __init__(self, initial_pos=400.0, initial_vel=7.8, dt=1.0, min_altitude=150):
        ...
        Args:
            ...
            dt (float): Time step in seconds (default: 1.0)
            min_altitude (float): Minimum allowable altitude in km (default: 150.0)
        """
    ...
    def update(self, measurement):
        ...
        if self.min_altitude is not None:
            self.kf.x[0] = max(self.min_altitude, self.kf.x[0])  # Apply minimum altitude constraint
